...
import { getBoardRoot, readManifest, loadAllCardFiles } from '@personal-kanban/core';
import type { Manifest } from '@personal-kanban/core';
...
export class MetricsPanel {
  ...
  private constructor(panel: vscode.WebviewPanel, workspaceRoot: string, extensionUri: vscode.Uri) {
    this._panel = panel;
    const boardFolderName = vscode.workspace.getConfiguration('personal-kanban').get<string>('boardFolderName', '.personal-kanban');
    this._boardRoot = getBoardRoot(workspaceRoot, boardFolderName);
  ...
  private _sendData(): void {
    try {
      ...
      (manifest as Manifest).tags = config.get<Manifest['tags']>('tags', {});
      (manifest as Manifest).tagColorTarget = config.get<Manifest['tagColorTarget']>('tagColorTarget', 'tag');
      (manifest as Manifest).showCardAge = config.get<boolean>('showCardAge', true);

      const cards = loadAllCardFiles(this._boardRoot, {
        cardsFolderPath: config.get<string>('cardsFolderPath', ''),
        archiveFolderPath: config.get<string>('archiveFolderPath', ''),
      });
