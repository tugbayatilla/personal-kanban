...
import type { Card, WebviewMessage, Logger, Manifest } from '@personal-kanban/core';
...
export class BoardPanel {
  ...
  public static createOrShow(context: vscode.ExtensionContext, workspaceRoot: string, channel: vscode.OutputChannel): void {
    ...
    BoardPanel.currentPanel = new BoardPanel(panel, workspaceRoot, context.extensionUri, channel);
    context.subscriptions.push({ dispose: () => BoardPanel.currentPanel?._dispose() });
    channel.show(true);
  }

  private constructor(panel: vscode.WebviewPanel, workspaceRoot: string, extensionUri: vscode.Uri, channel: vscode.OutputChannel) {
    this._panel = panel;
    this._boardRoot = getBoardRoot(workspaceRoot, this._getBoardFolderName());
  ...
  private _getBoardFolderName(): string {
    return vscode.workspace.getConfiguration('personal-kanban').get<string>('boardFolderName', '.personal-kanban');
  }

  private _getHookOptions(): { enableHooks: boolean; notifications: boolean } {
  ...
  private _readManifestWithSettings(): Manifest {
    const manifest = readManifest(this._boardRoot);
    const config = vscode.workspace.getConfiguration('personal-kanban');
    manifest.tags = config.get<Manifest['tags']>('tags', {});
    manifest.tagColorTarget = config.get<Manifest['tagColorTarget']>('tagColorTarget', 'tag');
    manifest.showCardAge = config.get<boolean>('showCardAge', true);
    return manifest;
  }
  ...
  private async _handleMessage(msg: WebviewMessage): Promise<void> {
    switch (msg.type) {
      ...
      case 'addCard': {
        ...
        }, this._logger, this._getHookOptions());
        this._sendState(id);
        break;
      }
      ...
      case 'saveCard': {
        ...
        if (existing && existing.content !== msg.content) {
          ...
          const manifest = this._readManifestWithSettings();
      ...
      case 'moveCard': {
        ...
        if (movedCard) {
          ...
          const hookOpts = this._getHookOptions();
      ...
      case 'archiveDone': {
        ...
        fireHook(this._boardRoot, m4, 'cards.archived', { column: m4.column_stamps?.done_at ?? 'done' }, this._logger, this._getHookOptions());
        this._sendState();
        break;
      }
    }
