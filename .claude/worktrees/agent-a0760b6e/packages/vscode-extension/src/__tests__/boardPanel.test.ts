...
import type { Card, WebviewMessage } from '@personal-kanban/core';
...
async function callHandler(
  ...
  const fakeChannel = { appendLine: jest.fn(), show: jest.fn(), dispose: jest.fn() };
  ...
  const instance = new PanelClass(fakeWebviewPanel as never, workspaceRoot, fakeExtensionUri as never, fakeChannel as never);
...
describe('BoardPanel message handler', () => {
  ...
  describe('external card move hook firing', () => {
    ...
    function makePanel(workspaceRoot: string): {
      ...
      const fakeChannel2 = { appendLine: jest.fn(), show: jest.fn(), dispose: jest.fn() };
      ...
      const instance = new PanelClass(fakeWebviewPanel as never, workspaceRoot, { fsPath: '/fake/extension' } as never, fakeChannel2 as never);
      return {
    ...
    it('fires card.moved hook when a card file is externally moved to a new column', async () => {
      const hooksModule = await import('@personal-kanban/core');
