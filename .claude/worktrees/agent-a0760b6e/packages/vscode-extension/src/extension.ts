...
import { getBoardRoot, boardExists, readManifest, writeManifest, withLock } from '@personal-kanban/core';
import { loadAllCardFiles, computeMetrics } from '@personal-kanban/core';
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('personal-kanban.exportMetrics', () => exportMetrics(channel))
  );
}
...
function makeBoardFolderName(): string {
...
function initBoard(context: vscode.ExtensionContext): void {
  ...
  const boardRoot = getBoardRoot(workspaceRoot, makeBoardFolderName());
...
function exportMetrics(channel: vscode.OutputChannel): void {
  const logger: Logger = {
    info: (msg) => channel.appendLine(`[INFO] ${msg}`),
    error: (msg) => channel.appendLine(`[ERROR] ${msg}`),
  };
  void logger; // logger available for future use
  ...
  try {
    ...
    // Inject VSCode settings into manifest
    const config = vscode.workspace.getConfiguration('personal-kanban');
  ...
  const cards = loadAllCardFiles(boardRoot, {
    cardsFolderPath: cfg.get<string>('cardsFolderPath', ''),
    archiveFolderPath: cfg.get<string>('archiveFolderPath', ''),
  });
