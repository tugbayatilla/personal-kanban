...
describe('readManifest', () => {
  ...
  it('reads a v1 manifest and returns default tags/tagColorTarget/showCardAge', () => {
    writeMinimalManifest(boardRoot);

    const manifest = readManifest(boardRoot);

    expect(manifest.tags).toEqual({});
    expect(manifest.tagColorTarget).toBe('tag');
    expect(manifest.showCardAge).toBe(true);
  });
