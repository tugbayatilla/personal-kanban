...
export function getBoardRoot(workspaceRoot: string, boardFolderName = '.personal-kanban'): string {
  return path.join(workspaceRoot, boardFolderName);
}
...
function getCardsDir(boardRoot: string, cardsFolderPath = ''): string {
  if (cardsFolderPath) {
    return path.resolve(boardRoot, cardsFolderPath);
  }
  return path.join(boardRoot, 'cards');
}

function getArchiveDir(boardRoot: string, archiveFolderPath = ''): string {
  if (archiveFolderPath) {
    return path.resolve(boardRoot, archiveFolderPath);
  }
...
export function readManifest(boardRoot: string): Manifest {
  ...
  // Tags, tagColorTarget, and showCardAge default to empty/defaults when not in manifest.
  if (!data.tags) data.tags = {};
  if (!data.tagColorTarget) data.tagColorTarget = 'tag';
  if (data.showCardAge === undefined) data.showCardAge = true;
