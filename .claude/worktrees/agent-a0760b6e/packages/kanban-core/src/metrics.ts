...
export function loadAllCardFiles(
  boardRoot: string,
  options: { cardsFolderPath?: string; archiveFolderPath?: string } = {}
): CardSummary[] {
  const cardsDir = options.cardsFolderPath
    ? path.resolve(boardRoot, options.cardsFolderPath)
    : path.join(boardRoot, 'cards');

  const archiveDir = options.archiveFolderPath
    ? path.resolve(boardRoot, options.archiveFolderPath)
